...
import {
  loadBinaryArtifacts,
  runBinaryInference,
} from '@/app/utils/binary-runtime';
import {
  loadMulticlassArtifacts,
  runMulticlassInference,
} from '@/app/utils/multiclass-runtime';
...
export default function ChatPage() {
  ...
  const [selectedModelSubclass, setSelectedModelSubclass] = useState<
    string | null
  >(null);
      ...
      if (selectedModel) {
        ...
        try {
          ...
          const modelPathPrefix = `models/${selectedModel.type}/${selectedModel.prefix}${
            selectedModel.subClasses.length && selectedModelSubclass
              ? `(${selectedModelSubclass})`
              : ''
          }`;

          // Load ONNX model
          const inferenceSession = await ort.InferenceSession.create(
            `${modelPathPrefix}/model.onnx`
          );
      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            <Tooltip title="Select model type" arrow placement="top">
              <FormControl
                size="small"
              ...
              >
                ...
                <Select
                  ...
                  onChange={e =>
                    handleSelectModelType(e.target.value as string)
                  }
                  sx={{
                ...
                >
                  ...
                  <MenuItem
                    key={ModelType.MULTICLASS}
                    value={ModelType.MULTICLASS}
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <SchemaOutlined fontSize="small" color="primary" />
            ...
            {!!selectedModel && selectedModel?.subClasses.length > 0 && (
              <Tooltip
              ...
              >
                ...
                >
                  ...
                  <Select
                    ...
                    onChange={e =>
                      handleSelectModelSubclass(e.target.value as string)
                    }
                    sx={{
                      bgcolor: 'rgba(255, 255, 255, 0.7)',
                      borderRadius: '8px',
